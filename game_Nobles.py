...
class NoblesClass:
    ...
    def list_names(self):
        noble_list = []
        for k, v in self.nobles.items():
            noble_list.append((self.nobles[k]["full_name"], k))
        return noble_list
    ...
    def retrieve_noble(self, ident):
        for stored_noble in self.nobles:
            stored_noble = self.nobles[stored_noble]
            if stored_noble["ident"] == ident:
                return stored_noble
        else:
            return False

    #asks for a Nobles name, then shows their stats and positions
    def get_stats(self, ident):
        noble_list = self.list_names()
        #request = self.Controller.get_input("Which Noble?", "str", noble_list)
        noble = self.retrieve_noble(ident)
        if noble:
            string_response = ""
            string_response += "Full Name:   %s\n" % noble["extended_title"]
            string_response += "Intelligence    %d  Ambition:    %d\nNetwork:        %d  Wealth:      %d\n" % (noble["intel"], noble["ambition"], noble["network"], noble["wealth"])
            try:
                string_response += "A member of the kings court: %s" % noble["position"]
            except KeyError:
                pass
            return string_response
        else:
            return "Noble does not exist!"
    ...
    def create_noble_random(self):
        ...
        maxid = 100
        for k, v in self.nobles.items():
            if int(self.nobles[k]["ident"]) > maxid:
                maxid = int(self.nobles[k]["ident"])
        ...
        self.nobles[ident] = stats

        self.save_file()
        string_response = "New noble added!\nName: %s\nIntelligence: %d\nAmbition: %d\nNetwork: %d\nWealth: %s" % (extended_title, intel, ambition, network, wealth)
        return string_response
    ...
    def remove_noble(self, ident):
        noble = self.retrieve_noble(ident)
        try:
            response = noble["full_name"] + " executed!"
            del self.nobles[ident]
            self.save_file()
            return response
        except (KeyError, TypeError):
            return "That's noble doesn't exist!"
...
if __name__ == "__main__":
    ...
    nobles = NoblesClass()
    print(nobles.list_names())
    var = nobles.create_noble_random()
    print(var)
